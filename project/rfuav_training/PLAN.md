...
Step 2: 预处理
  - 降采样：100 MHz → 60 MHz
  - STFT 生成频谱图（fs=60 MHz）
  - **动态 bbox 标注**（基于信号检测算法，详见 §3.5）

### 3.5 动态 bbox 标注策略

> ⚠️ **固定 bbox 会导致 YOLO 训练无效**——所有样本用相同 bbox 会让模型退化为"伪装成检测的分类"。必须使用动态 bbox。
...
STFT 频谱图 (dB 格式)
    ↓
① 噪声底估计（15% 分位数，按频率轴）
② 能量阈值分割：P_db > noise_floor + 6dB → 二值 mask
③ 形态学清理（开闭运算 + 面积滤波）
④ 连通域分析 → 每个连通域外接矩形
⑤ 外扩 10% margin → 归一化为 YOLO 格式 (cx, cy, w, h)
```

**关键参数：**
| 参数 | 值 |
|---|---|
| 噪声底估计 | 每频率bin列的15%分位数 |
| 能量阈值 | noise_floor + 6 dB |
| 形态学结构元 | 3×3 全1 |
| 最小连通域面积 | 20 像素 |
| bbox外扩margin | 10% |

**阈值选择理由：**
- DJI 信号比噪声高 15-40 dB，6dB 阈值在保证召回的同时抑制噪声
- 用噪声底自适应替代固定阈值，适应不同 SNR 环境

**Fallback：** 若某样本检测不到任何连通域（纯噪声图），则该样本不进入训练集
...
| **业务需求** | **必须识别具体机型**（Stage2 不可省略）| 2026-05-13 |
...
| **推理 burst** | **burst=200**（推荐起点，可调）| 2026-05-13 |

---

## 七、推理与采数要求

> ⚠️ **训练与推理必须严格对齐**，以下参数从训练计划固化，推理和采数必须遵循。

### 7.1 Pluto 推理参数

| 参数 | 值 | 说明 |
|---|---|---|
| 采样率 | 60 MHz | 与训练降采样后一致 |
| 中心频率（5.8GHz）| 5760 / 5775 / 5800 / 5825 / 5850 MHz | 多频点轮询 |
| 中心频率（2.4GHz）| 2450 / 2470 MHz | 多频点轮询 |
| **burst 数** | **200**（推荐起点）| 等效窗口 ~3.4 ms，SNR 50+ dB |
| STFT nperseg | 1024 | 与训练一致 |
| STFT hop | 512 | 与训练一致 |
| STFT window | hamming | 与训练一致 |
| STFT fs | 60 MHz | 与训练一致 |
...
| 频段 | 中心频率 | Pluto 轮询范围 |
|---|---|---|
| 5.8 GHz | 5760-5850 MHz | 5 个频点轮询 |
| 2.4 GHz | 2450-2470 MHz | 2 个频点轮询 |

> **注意**：DJI Mini3 同时在 2.4GHz（遥控器）和 5.8GHz（图传）有信号，两个频段都需要检测。

### 7.4 采数与训练的闭环要求

| 要求 | 说明 |
|---|---|
| **禁止用 Pluto 实采数据训练** | `pluto_raw/` 下所有数据仅用于推理验证 |
| **推理 STFT 必须与训练一致** | fs/nperseg/hop/window 必须完全一致 |
| **burst 配置影响等效窗口** | burst=200 时等效窗口 ~3.4 ms，与训练窗口大致对齐 |

---

## 八、待讨论/待确认

> 训练计划中所有决策项已全部确认，无待讨论项。
> 
> RFUAV 全集（37 机型）暂不联系作者。
